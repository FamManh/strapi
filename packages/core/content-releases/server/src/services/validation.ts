...
import type { CreateReleaseAction, ReleaseAction } from '../../../shared/contracts/release-action';
...
const createReleaseValidationService = ({ strapi }: { strapi: LoadedStrapi }) => ({
  async validateUniqueEntry(
    ...
    if (!release) {
      throw new errors.ValidationError(`No release found for id ${id}`);
    }
    ...
    if (isEntryInRelease) {
      throw new errors.ValidationError(
        `Entry with id ${releaseActionArgs.entry.id} and contentType ${releaseActionArgs.entry.contentType} already exists in release with id ${id}`
      );
    }
  },
  validateEntryContentType(contentTypeUid: ReleaseAction['contentType']) {
    const contentType = strapi.contentType(contentTypeUid);

    if (!contentType) {
      throw new errors.ValidationError(`No content type found for uid ${contentTypeUid}`);
    }
